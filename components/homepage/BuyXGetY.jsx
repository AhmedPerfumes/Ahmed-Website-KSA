...
export default function BuyXGetY() {
    ...
    const { addProductToCart, toggleWishlist, isAddedtoWishlist, cartProducts, setCartProducts } = useContextElement();

    /* Fire the same CartToast used across the shop */
    ...
    /* ── Hide completely when no active promotions exist ── */
    ...
    return (
        <section className="bxy-section" aria-label="Buy X Get Y Offers" id="buy-x-get-y">
            <div className="bxy-inner">
                                ...
                                <SwiperSlide key={elm.product_id}>
                                    <article className="bxy-card">
                                        ...
                                        <div className="bxy-card__media">
                                            ...
                                            <button
                                                type="button"
                                                className={`bxy-card__wish${inWish ? " active" : ""}`}
                                        ...
                                        <div className="bxy-card__info">
                                            ...
                                            <div className="bxy-card__price">
                                                <span className="bxy-card__price-cur">{fmt(base)}</span>
                                            </div>
                                            ...
                                            {elm.product_qty > 0 ? (() => {
                                                ...
                                                return cartQty > 0 ? (
                                                    <div className="bxy-card__stepper">
                                                        <button type="button" className="bxy-card__step-btn" aria-label="Decrease" onClick={() => {
                                                        ...
                                                        <span className="bxy-card__step-num">{cartQty}</span>
                                                        <button type="button" className={`bxy-card__step-btn${cartQty >= maxQty ? " bxy-card__step-btn--max" : ""}`} aria-label="Increase" onClick={() => {
                                                    ...
                                                    <button type="button" className="bxy-card__atc" onClick={() => { addProductToCart({ ...elm, category_name: elm.category_name, subcategory_name: elm.subcategory?.subcategory_name, _silent: true }); fireToast(elm, 1); }}>{t("Add To Cart")}</button>
                                                );
                                            })() : (
                                                <span className="bxy-card__atc bxy-card__atc--out">{t("Out Of Stock")}</span>
                                            )}
                                        </div>
                ...
                <div className="bxy-nav">
                    <button type="button" ref={prevRef} className="bxy-arrow" aria-label="Previous">
                        <svg viewBox="0 0 24 24"><path d="M15 18l-6-6 6-6"/></svg>
                    </button>
                    <button type="button" ref={nextRef} className="bxy-arrow" aria-label="Next">

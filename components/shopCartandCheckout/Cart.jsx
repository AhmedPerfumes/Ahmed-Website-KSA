 ...
 *
 * UX changes (round 3):
 *  A  Shipping bar sits ABOVE the Order Summary card (outside collapsible area)
 *  B  Promo code = cc-coupon-card with cc-coupon-label-row + "View Offers" — identical
 *     to the checkout RIGHT sidebar (always-visible input, not a collapsible toggle)
 *  C  Tabby: script loaded once on mount; widget re-inited whenever cart changes
 *  D  Delivery warning contrast: dark warm text #5a4a1a for WCAG AA
 */
...
export default function Cart() {
  ...
  // (B) Coupon state — identical to Checkout right-sidebar
  ...
  // (C) Tabby: load script once on mount
  useEffect(() => {
    const scriptId = "tabby-promo-cart";
  ...
  // (C) Re-init Tabby widget whenever cart or totals change
  useEffect(() => {
    ...
    // 800ms gives the script time to hydrate after cart update
    const t = setTimeout(init, 800);
    return () => clearTimeout(t);
  }, [cartProducts, locale]); // re-runs on every cart change (add / remove / qty)
  ...
  const removeCoupon = () => {
    setCouponData(null); setCouponCode(""); setCouponError(null); setCouponSuccess(null);
  };
  ...
  return (
    <div className="shopping-cart" style={{ minHeight: "calc(100vh - 300px)", paddingBottom: "70px" }}>
        ...
        <>
          ...
          <div className="cart-table__wrapper">
            <div className="cc-cart-items-list">
              ...
              <div className="cc-cart-continue">
                <Link href={`/${locale}/shop`} className="cc-cart-continue__link">← Continue Shopping</Link>
              </div>
                ...
                return (
                  <div key={i} className="cc-cart-item">
                    <div className="cc-cart-item__image">
                      {pdpHref
                        ? <Link href={pdpHref}><Image src={imgSrc} width={80} height={80} alt={he.decode(elm.product_name || "")} loading="lazy" style={{ objectFit: "cover", borderRadius: "6px" }} /></Link>
                        : <Image src={imgSrc} width={80} height={80} alt={he.decode(elm.product_name || "")} loading="lazy" style={{ objectFit: "cover", borderRadius: "6px" }} />}
                    </div>
                    <div className="cc-cart-item__body">
                      {pdpHref
                        ? <Link href={pdpHref} className="cc-cart-item__name cc-cart-item__name--link">{he.decode(elm.product_name || "")}</Link>
                      ...
                      <div className="cc-cart-item__price-row">
                        {disc !== null ? (
                          <><span className="cc-cart-item__price-old">{currency.symbol}{parseFloat(elm.price).toFixed(2)}</span><span className="cc-cart-item__price-sale">{currency.symbol}{disc.toFixed(2)}</span></>
                        ) : (
                      ...
                      {!isGift ? (
                        <div className="cc-cart-item__qty-row">
                          <button type="button" className="cc-cart-item__qty-btn" aria-label="Decrease" onClick={() => setQuantity(elm.product_id, elm.quantity - 1, elm.product_qty, elm?.maximum_order_quantity)}>−</button>
                          <span className="cc-cart-item__qty-num">{elm.quantity}</span>
                          <button type="button" className="cc-cart-item__qty-btn" aria-label="Increase" onClick={() => setQuantity(elm.product_id, elm.quantity + 1, elm.product_qty, elm?.maximum_order_quantity)}>+</button>
                        </div>
                      ) : (
                        <span style={{ fontSize: "0.72rem", color: "#888" }}>Qty: 1</span>
                      )}
                      <span className="cc-cart-item__subtotal">Subtotal: <strong>{currency.symbol}{getItemSubtotal(elm)}</strong></span>
                    </div>
                    {!isGift && (
                      <button type="button" className="cc-cart-item__remove" aria-label="Remove item" onClick={() => removeItem(elm.product_id)}>Remove</button>
                    )}
                  </div>
                );
              })}
          ...
          <div className="shopping-cart__totals-wrapper">
            <div className="sticky-content">
              ...
              {/* (A) Shipping bar — OUTSIDE the collapsible card, always visible */}
              <div className="cc-cart-ship-bar">
                ...
                {/* (D) Delivery warning — high-contrast warm dark text */}
              ...
              {/* (B) Collapsible Order Summary card */}
              <div className="cc-order-summary-right">
                <div className="cc-osr__header" onClick={() => setSummaryOpen(v => !v)} role="button" tabIndex={0}
                  ...
                  <div className="cc-osr__header-right">
                    ...
                    <svg className={`cc-osr__chevron${summaryOpen ? " cc-osr__chevron--open" : ""}`} width="12" height="8" viewBox="0 0 12 8" fill="none" aria-hidden="true">
                ...
                {summaryOpen && (
                  <div className="cc-osr__body">
                    <div className="cc-osr__col-heads"><span>Product</span><span>Subtotal</span></div>
                    {cartProducts.map((elm, i) => {
                      const disc = getDiscountedPrice(elm);
                      return (
                        <div key={i} className="cc-osr__row">
                          <span className="cc-osr__product-name">{he.decode(elm.product_name)} <strong>×{elm.quantity}</strong></span>
                          <span className="cc-osr__product-price">
                            {disc !== null ? (
                              <><span className="cc-price-old">{currency.symbol}{(elm.price * elm.quantity).toFixed(2)}</span><span className="cc-price-sale">{currency.symbol}{(disc * elm.quantity).toFixed(2)}</span></>
                            ) : (
                    ...
                    <div className="cc-osr__row cc-osr__row--sub"><span>Subtotal</span><span>{totalPrice.toFixed(2)} {currency.symbol}</span></div>
                    <div className="cc-osr__row cc-osr__row--sub">
                      <span>Shipping</span>
                      <span>{freeShippingFlag ? <span className="cc-osr__free">🎉 Free</span> : shippingLoaded ? `${shippingServiceCharges[0].price} ${currency.symbol}` : "…"}</span>
                    </div>
                    {couponData && (
                    ...
                    {/* (C) Tabby widget — re-inits on cart change via useEffect */}
              ...
              {/* (B) Promo code — identical to Checkout right-sidebar cc-coupon-card pattern */}
              <div className="cc-coupon-card">
                {couponData ? (
                  ...
                  /* Exact replica of Checkout right sidebar — label row + input always visible */
                  <>
                    <div className="cc-coupon-label-row">
                      <span className="cc-coupon-label-text">Have a Promo Code?</span>
                      <button type="button" className="cc-coupon-view-offers" onClick={() => setShowCouponModal(true)}>
                        View Offers
                      </button>
                    </div>
              ...
              {/* Checkout CTA */}
              <Link href={`/${locale}/shop-checkout`} className="cc-btn-gold"
              ...
              <p className="cc-totals-card__pay-note">🔒 Visa · Mastercard · Mada · Tamara · Tabby</p>
            </div>
          </div>

          {/* Coupon offers modal (same structure as Checkout — fetches coupons) */}
          {showCouponModal && (
            <div className="coupon-modal-overlay" onClick={() => setShowCouponModal(false)}>
              <div className="coupon-modal" onClick={e => e.stopPropagation()}>
                ...
                <div className="coupon-empty">Sign in to view your available coupons.</div>
              </div>
            </div>
          )}
        </>
      ...
      <div className="cc-ymal-wrapper"><YouMayAlsoLike /></div>
    </div>
  );
}

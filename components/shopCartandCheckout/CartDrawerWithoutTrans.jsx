...
export default function CartDrawerWithoutTrans() {
  ...
  const getItemPrice = (elm) => {
    ...
    if (elm?.discount) {
      if (new Date(current_date_time) >= new Date(elm.discount.start_date) && new Date(current_date_time) <= new Date(elm.discount.end_date)) {
        if (elm.discount.discount_type === "percent") {
          ...
          return { total: (salePrice * elm.quantity).toFixed(2), unitSale: parseFloat(salePrice), hasDiscount: true, originalPrice: parseFloat(elm.price).toFixed(2) };
    ...
    if (!elm?.discount && elm?.sale_price && Number(elm.sale_price) > 0 && Number(elm.sale_price) < Number(elm.price)) {
      const salePrice = parseFloat(elm.sale_price).toFixed(2);
      return { total: (parseFloat(salePrice) * elm.quantity).toFixed(2), unitSale: parseFloat(salePrice), hasDiscount: true, originalPrice: parseFloat(elm.price).toFixed(2) };
    }
    return { total: (elm.price * elm.quantity).toFixed(2), unitSale: null, hasDiscount: false, originalPrice: null };
  };
      ...
      <div className="aside aside_right overflow-hidden cart-drawer cd-new" id="cartDrawer">
        ...
        {cartProducts.length > 0 ? (
                      ...
                      <div className="cd-item__price-row">
                        {priceInfo.hasDiscount ? (
                          <>
                            ...
                            <span className="cd-item__price-sale">{priceInfo.unitSale?.toFixed(2)} {currency?.symbol}</span>
                          </>
                        ) : (
                          <span className="cd-item__price">{parseFloat(elm.price).toFixed(2)} {currency?.symbol}</span>
                        )}
                      </div>
                      {!elm.is_gift ? (
                        <div className="cd-qty">
                          <button className="cd-qty__btn" onClick={() => setQuantity(elm.product_id, elm.quantity - 1, elm.product_qty, elm?.maximum_order_quantity)} disabled={elm.quantity <= 1} type="button">−</button>
                          <span className="cd-qty__num">{elm.quantity}</span>
                          <button className="cd-qty__btn" onClick={() => setQuantity(elm.product_id, elm.quantity + 1, elm.product_qty, elm?.maximum_order_quantity)} type="button">+</button>
                        </div>
                      ) : (
                        <span className="cd-item__gift-badge">🎁 Free Gift</span>
                      )}
                    </div>
                    <button className="cd-item__remove" onClick={() => removeItem(elm.product_id)} type="button">
          ...
          <div className="cd-empty">
            ...
            <Link href="/shop" className="cd-btn cd-btn--gold" onClick={closeCart}>Explore Collection</Link>
          </div>
        )}
        ...
        {cartProducts.length > 0 && (
          <div className="cd-footer">
            ...
            <div className="cd-ctas">
              <Link href="/shop-cart" className="cd-btn cd-btn--outline" onClick={closeCart}>View Cart</Link>
              <Link href="/shop-checkout" className="cd-btn cd-btn--gold" onClick={closeCart}>Checkout</Link>
            </div>
            <p className="cd-payment-note">🔒 Secure checkout · Visa · Mastercard · mada · Tamara · Tabby</p>
          </div>
        )}
      </div>

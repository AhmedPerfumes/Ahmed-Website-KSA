...
class AhmedTrackerSDK {
  ...
  captureUtmParams() {
    try {
      ...
      // ---- Auto-Detect Abandoned Cart Recovery Email Click ----
      const recoverySessionId = urlParams.get('recovery_session_id');
      if (recoverySessionId) {
        currentUtm.recovery_session_id = recoverySessionId;
        foundNew = true;
      }
  ...
  attachEventListeners() {
    ...
    // Throttled scroll depth listener
  ...
  handleScroll() {
    ...
    // Optimized: Only track significant milestones (50% and 90%) to prevent DB bloat
  ...
  trackTimeOnPage() {
    ...
    // Optimized: Ignore micro bounces (< 5 seconds) and cap max idle tab duration to 1800s (30m)
  ...
  track(eventName, eventData = {}) {
    ...
    try {
      ...
      if (userStr) {
        ...
        customerPhone = user.phone || user.customer_phone || user.phone_number || null;
      }
    } catch (e) {}

    if (eventData && (eventData.phone || eventData.customer_phone)) {
      customerPhone = eventData.phone || eventData.customer_phone;
    }

    if (this.utmParams && this.utmParams.recovery_session_id && typeof eventData === 'object' && eventData !== null) {
      eventData.recovery_session_id = this.utmParams.recovery_session_id;
    }
